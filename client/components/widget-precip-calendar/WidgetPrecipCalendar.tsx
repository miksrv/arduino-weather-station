...
const getCellColor = (total: number): string => {
    if (total <= 0) return 'var(--input-border-color)'
    if (total <= 1) return '#cce5ff'
    if (total <= 5) return '#66b2ff'
    if (total <= 20) return '#1a7fd4'
    return '#004080'
}
...
const WidgetPrecipCalendar: React.FC<WidgetPrecipCalendarProps> = ({ loading, year, days }) => {
    ...
    if (loading) {
        return <Skeleton style={{ width: '100%', height: 260 }} />
    }
        ...
        <div className={styles.widget}>
            <div className={styles.grid}>
                ...
                {MONTH_ABBRS.map((abbr, monthIndex) => {
                    ...
                    return (
                        <div key={abbr} className={styles.monthRow}>
                            <div className={styles.monthLabel}>{abbr}</div>
                            {DAY_NUMBERS.map((day) => {
                                if (day > maxDays) {
                                    return <div key={day} className={styles.cellEmpty} />
                                }
            ...
            <div className={styles.legend}>
                <div className={styles.legendItem}>
                    <span className={styles.legendSwatch} style={{ backgroundColor: 'var(--input-border-color)' }} />
                    <span className={styles.legendLabel}>{t('dry-days')}</span>
                </div>
                <div className={styles.legendItem}>
                    <span className={styles.legendSwatch} style={{ backgroundColor: '#cce5ff' }} />
                    <span className={styles.legendLabel}>{t('precip-level-trace')}</span>
                </div>
                <div className={styles.legendItem}>
                    <span className={styles.legendSwatch} style={{ backgroundColor: '#66b2ff' }} />
                    <span className={styles.legendLabel}>{t('precip-level-light')}</span>
                </div>
                <div className={styles.legendItem}>
                    <span className={styles.legendSwatch} style={{ backgroundColor: '#1a7fd4' }} />
                    <span className={styles.legendLabel}>{t('precip-level-moderate')}</span>
                </div>
                <div className={styles.legendItem}>
                    <span className={styles.legendSwatch} style={{ backgroundColor: '#004080' }} />
